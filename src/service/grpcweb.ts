...
export function generateGrpcWebService(
  ...
  return [
    ...
    createFile(
      generateJavaScript(descriptor, exportMap),
      `${filename}_service.js`
    ),
  ]
}
...
function generateTypeScriptDefinition(
  ...
  serviceDescriptor.services.forEach((service) => {
    ...
    service.methods.forEach((method) => {
      printer.printLn(
        `type ${method.serviceName}${method.nameAsPascalCase} = {`
      )
      ...
      printer.printIndentedLn(
        `readonly requestStream: ${method.requestStream};`
      )
      printer.printIndentedLn(
        `readonly responseStream: ${method.responseStream};`
      )
      printer.printIndentedLn(
...
function printUnaryStubMethod(
  ...
  printer
    ...
    .printLn(`metadata: metadata || this.options.metadata,`)
...
function printClientStreamStubMethod(
  ...
  printer
    ...
    .indent()
    .printLn(`client.start(metadata || this.options.metadata);`)
    .dedent()
    .printLn(`}`)
